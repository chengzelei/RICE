...
class RndNetwork(nn.Module):
    ...
    def __init__(self, obs_shape: Union[int, SequenceType], hidden_size_list: SequenceType) -> None:
        ...
        if isinstance(obs_shape, int) or len(obs_shape) == 1:
            ...
            self.target = ConvEncoder(obs_shape=obs_shape, hidden_size_list=hidden_size_list, kernel_size = [4, 4, 3])
            self.predictor = ConvEncoder(obs_shape=obs_shape, hidden_size_list=hidden_size_list, kernel_size = [4, 4, 3])
        else:
            raise KeyError(
