...
env = DummyVecEnv([lambda: make_retrain_env("HalfCheetah-v3", random_net, bonus_scale, inv_cov)])
env = VecMonitor(env, "weak_retrain_log/")
#env = Monitor(env, "retrain_log/")
eval_callback = EvalCallback(env, best_model_save_path='weak_retrain_models/best_model',
                             log_path='weak_retrain_models/results', eval_freq=10000)
...
model = PPO.load("/home/zck7060/Retrain_mujoco/halfcheetah/baseline/weak_models/best_model/best_model", env = env, custom_objects=custom_objects, tensorboard_log="weak_retrain_log/")
