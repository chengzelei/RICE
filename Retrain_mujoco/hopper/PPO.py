...
class PPO(OnPolicyAlgorithm):
    ...
    def train(self) -> None:
        ...
        for epoch in range(self.n_epochs):
            ...
            for rollout_data in self.rollout_buffer.get(self.batch_size):
                ...
                if self.bonus == 'e3b':
                    ...
                    dynamic_loss = F.mse_loss(pred_actions, true_actions)
                    dynamic_losses.append(dynamic_loss.item())
                    loss = policy_loss + self.ent_coef * entropy_loss + self.vf_coef * value_loss + dynamic_loss
                    ...
                    forward_losses.append(np.mean(forward_losses))
        ...
        if self.bonus == 'e3b':
            self.logger.record("train/inverse_dynamic_loss", np.mean(dynamic_losses))
        elif self.bonus == 'rnd':
            self.logger.record("train/forward_loss", np.mean(forward_losses))
