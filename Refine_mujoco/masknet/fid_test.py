...
def select_critical_steps (critical=True, ratio=0.3):
    ...
    for i_episode in range(500):
        mask_probs_path = "./hopper_recording/mask_probs_" + str(i_episode) + ".out"
        mask_probs = np.loadtxt(mask_probs_path)

        confs = mask_probs

        iteration_ends_path = "./hopper_recording/eps_len_" + str(i_episode) + ".out"
    ...
    if critical:
        np.savetxt("./hopper_recording/critical_steps_starts.out", steps_starts)
        np.savetxt("./hopper_recording/critical_steps_ends.out", steps_ends)
    else:
        np.savetxt("./hopper_recording/non_critical_steps_starts.out", steps_starts)
        np.savetxt("./hopper_recording/non_critical_steps_ends.out", steps_ends)        


def replay(env, game_num, base_model, step_start, step_end, random_replace, orig_traj_len):
    action_seq = np.load("./hopper_recording/act_seq_" + str(game_num) + ".npy", allow_pickle=True)
...
def cal_fidelity_score(critical_ratios, results, replay_results):
    ...
    for j in range(len(p_ls)):
        p_ds.append(np.abs(results[j]-replay_results[j])/2000)
...
env = VecMonitor(env, "hopper_recordings/")
env = VecNormalize.load("/home/zck7060/Retrain_based_on_explanation/Retrain_mujoco/mujoco/halfcheetah/near_optimal/model/vec_normalize.pkl", env)
env.training = False

base_model = PPO.load("/home/zck7060/Retrain_based_on_explanation/Retrain_mujoco/mujoco/halfcheetah/near_optimal/model/best_model")

results = np.loadtxt("./hopper_recording/reward_record.out")
...
for i in range(len(ratios)):
    ...
    critical_steps_starts = np.loadtxt("./hopper_recording/critical_steps_starts.out")
    critical_steps_ends = np.loadtxt("./hopper_recording/critical_steps_ends.out")
    ...
    for game_num in range(500):
        orig_traj_len = np.loadtxt("./hopper_recording/eps_len_"+ str(game_num) + ".out")
    ...
    fid_score = cal_fidelity_score(critical_ratios, results, replay_results)
    fid_scores_random.append(fid_score)
    print("Fidelity score: ", fid_score)

    select_critical_steps(critical=False, ratio=ratios[i])
    
    non_critical_steps_starts = np.loadtxt("./hopper_recording/non_critical_steps_starts.out")
    non_critical_steps_ends = np.loadtxt("./hopper_recording/non_critical_steps_ends.out")
...
print("Fidelity score: ", fid_scores_random)
