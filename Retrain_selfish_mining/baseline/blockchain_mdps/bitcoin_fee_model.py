...
class BitcoinFeeModel(BlockchainModel):
    def __init__(self, alpha: float, gamma: float, max_fork: int, fee: float, transaction_chance: float, max_pool: int):
        ...
        self.truncate_reward = self.alpha
        super().__init__()
    ...
    def get_truncate_reward(self,state: BlockchainModel.State) ->float:
        a, h, fork, pool, length_a, length_h, transactions_a, transactions_h = self.dissect_state(state)
        accepted_blocks = length_a
        action_param = length_a
        accepted_transactions = self.chain_transactions(self.truncate_chain(a, action_param))
        reward = (accepted_blocks + accepted_transactions * self.fee) * self.block_reward * self.truncate_reward
        return reward
...
if __name__ == '__main__':
    ...
    mdp = BitcoinFeeModel(0.35, 0.5, 10, fee=10, transaction_chance=0.1, max_pool=10)
