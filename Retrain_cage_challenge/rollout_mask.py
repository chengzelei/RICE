...
def train(env, input_dims, action_space,
    ...
    ckpt = '/home/jys3649/projects/xai_masknet/cyborg-cage-2/Models/bline_mask/20000.pth'
    agent = PPOAgent_mask(input_dims, action_space, lr, betas, gamma, K_epochs, eps_clip, start_actions=start_actions, restore=True, ckpt=ckpt, deterministic=True)
    ...
    for i_episode in range(1, max_episodes + 1):
        ...
        for t in range(max_timesteps):
            time_step += 1
            action, additional_reward = agent.get_action(state)
        ...
        if i_episode % print_interval == 0:
            print('Episode {} \t Avg reward: {}'.format(i_episode, np.mean(np.array(rewards))))
