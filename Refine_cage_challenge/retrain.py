...
from Wrappers.ChallengeWrapper2_retrain import ChallengeWrapper2_Retrain
import inspect
from Agents.PPOAgent_Retrain import PPOAgent_Retrain
import random
...
def train(env, input_dims, action_space,
          ...
          gamma, lr, betas, ckpt_folder, bonus_scale=0.01, print_interval=10, save_interval=100, start_actions=[]):


    ckpt = 'Models/bline/99800.pth'
    agent = PPOAgent_Retrain(input_dims, action_space, lr, betas, gamma, K_epochs, eps_clip, start_actions=start_actions, restore=True, ckpt=ckpt)
    ...
    for i_episode in range(1, max_episodes + 1):
        ...
        for t in range(max_timesteps):
            ...
            bonus = agent.rnd_model.compute_bonus(state)
            reward += bonus_scale * bonus
        ...
        if i_episode % print_interval == 0:
            ...
            print('Episode {} \t Training Avg Reward: {}'.format(i_episode, running_reward))
            test_rewards = 0
            for _ in range(5):
                state = env.reset()
                for t in range(max_timesteps):
                    ...
                    test_rewards += reward
                agent.end_episode()
            print('Episode {} \t Test Avg Reward: {}'.format(i_episode, running_reward))
...
if __name__ == '__main__':
    ...
    env = ChallengeWrapper2_Retrain(env=CYBORG, agent_name="Blue")
