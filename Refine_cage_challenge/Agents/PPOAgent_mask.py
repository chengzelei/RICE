...
class PPOAgent_mask(BaseAgent):
    def __init__(self, input_dims=52, action_space=[0, 1], lr=0.002, betas=[0.9, 0.990], gamma=0.99, K_epochs=4, eps_clip=0.2, restore=False, ckpt=None,
    ...
    def get_mask_prob(self, observation):
    ...
    def get_action(self, observation, action_space=None):
        ...
        observation = self.pad_observation(observation)
        state = torch.FloatTensor(observation.reshape(1, -1)).to(device)
        baseline_action = self.baseline_policy.act(state, self.memory, deterministic=True)
        mask_action = self.old_policy.act(state, self.memory, deterministic=self.deterministic)
        if mask_action == 0:
            action = baseline_action
        else:
            action = np.random.randint(0, len(self.action_space))
            self.mask_this_state = True
            self.mask_count += 1
        action_ = self.action_space[action]

        if self.step_count >= 1000:
            print("mask ratio: ", self.mask_count/self.step_count)
            self.step_count = 0
            self.mask_count = 0
        ...
        additional_reward = self.lazy_reward if self.mask_this_state else 0
        return action_, self.lazy_reward
        # else:
        #     return action_
    ...
    def set_initial_values(self, action_space, observation=None):
        ...
        self.policy = ActorCritic(self.input_dims, self.n_actions_mask).to(device)
        if self.restore:
            pretained_model = torch.load(self.ckpt, map_location=lambda storage, loc: storage)
            self.policy.load_state_dict(pretained_model)
        self.optimizer = torch.optim.Adam(self.policy.parameters(), lr=self.lr, betas=self.betas)

        self.old_policy = ActorCritic(self.input_dims, self.n_actions_mask).to(device)
        self.old_policy.load_state_dict(self.policy.state_dict())

        self.baseline_policy = ActorCritic(self.input_dims, self.n_actions).to(device)
        self.baseline_policy.load_state_dict(torch.load('/home/jys3649/projects/xai_masknet/cyborg-cage-2/Models/bline/99800.pth'))
