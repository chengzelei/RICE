...
def select_critical_steps (critical=True, ratio=0.3):
    ...
    if critical:
        np.savetxt("./recording/critical_steps_starts.out", steps_starts)
        np.savetxt("./recording/critical_steps_ends.out", steps_ends)
    else:
        np.savetxt("./recording/non_critical_steps_starts.out", steps_starts)
        np.savetxt("./recording/non_critical_steps_ends.out", steps_ends)        


def replay(env, game_num, base_model, step_start, step_end, random_replace, orig_traj_len):
    action_seq = np.load("./recording/act_seq_" + str(game_num) + ".npy", allow_pickle=True)
...
results = np.loadtxt("./recording/reward_record.out")
...
for i in range(len(ratios)):
    ...
    for game_num in range(500):
        orig_traj_len = np.loadtxt("./recording/eps_len_"+ str(game_num) + ".out")
